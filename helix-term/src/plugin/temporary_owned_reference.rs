...
impl<A, B, C> MutTuple for (&mut A, &mut B, &mut C) {
    type EachAsTemporaryBorrowedMut = (
        TemporaryOwnedBorrowMut<A>,
        TemporaryOwnedBorrowMut<B>,
        TemporaryOwnedBorrowMut<C>,
    );
...
impl<A, B, C, D> MutTuple for (&mut A, &mut B, &mut C, &mut D) {
    type EachAsTemporaryBorrowedMut = (
        TemporaryOwnedBorrowMut<A>,
        TemporaryOwnedBorrowMut<B>,
        TemporaryOwnedBorrowMut<C>,
        TemporaryOwnedBorrowMut<D>,
    );
    ...
    fn wrap_in_temporary_borrowed_mut(self) -> Self::EachAsTemporaryBorrowedMut {
        (
            ...
            TemporaryOwnedBorrowMut { inner: self.1 as _ },
            TemporaryOwnedBorrowMut { inner: self.2 as _ },
            TemporaryOwnedBorrowMut { inner: self.3 as _ },
        )
    }
}
...
mod tests {
    ...
    fn single() {
        ...
        let ret = TemporaryOwnedBorrowMut::provide_with(&mut my_state, |mut my_state_owned| {
            *my_state_owned += 1;
